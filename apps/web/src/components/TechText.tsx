...
export const TechText: React.FC<TechTextProps> = ({
  ...
  useEffect(() => {
    ...
    const family = (s: any) => s.fontFamily || getComputedStyle(container).fontFamily || 'sans-serif';
    const fontFor = (s: any, size: number) => `${s.fontWeight} ${size}px ${family(s)}`;

    const setFont = (target: any, s: any, size: number) => {
    ...
    const sprite = (s: any, view: any, glyph: any, stroke: boolean) => {
    ...
    const falloff = (target: any, cx: number, cy: number, radius: number, strength: number, softness: number) => {
    ...
    const blit = (target: any, art: any, dx: number, dy: number, originX: number, originY: number) => {
      target.drawImage(
        art.image,
