...
export function evaluateHardRelevance(
  ...
  (assumption.concepts || []).forEach(concept => {
    ...
    const hasConceptHit = conceptWords.some(cw => docText.includes(cw));
    if (hasConceptHit) {
  ...
  (assumption.keywords || []).forEach(kw => {
    ...
    if (kwWords.some(kww => docText.includes(kww))) {
      keywordHits++;
    }
  });
  ...
  let score = Math.round(
    lexicalCoverage * 40 +
    (matchedEntities.length > 0 ? Math.min(30, matchedEntities.length * 15) : 0) +
    (matchedConcepts.length > 0 ? Math.min(20, matchedConcepts.length * 10) : 0) +
    (keywordHits > 0 ? Math.min(10, keywordHits * 5) : 0) +
    (hasSpecificObservation ? 10 : 0)
  );
  ...
  } else if (score >= 40 && (matchedEntities.length > 0 || keywordHits > 0)) {
    decision = 'UNCERTAIN';
